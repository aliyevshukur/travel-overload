...
export const Create = () => {
    ...
    const [selected, setSelected] = useState({ fieldId: null, text: "", start: null, end: null });
    ...
    return (
        <div className={'create-container'} onClick={(e) => clickedAside(e)}>
            <div className={'header'}>
                <div className={isDetails ? 'back' : 'title'}>
                    {
                        !isDetails ?
                            <>
                                ...
                                <input className={'title-input'} type={'text'} placeholder={'Başlığı daxil edin'} />
                            </>
                            :
                            <div className={'back-btn'} onClick={() => setDetails(false)}>
                                <CustomSvg name={'chervonLeft'} width={'12'} height={'22'} color={'#18A0FB'} />
                ...
                <div className={'actions'}>
                    <div className={'actions-post'}>
                        ...
                        <CustomSvg name={'send'} width={'25'} height={'25'} color={'#FFFFFF'} />
                    </div>
                    {
                        !isDetails ?
            ...
            <div className={'content'} onClick={(e) => clickedAside(e)}>
                {tools.visibility ?
                    <div className={'tools'} style={{ top: `${tools.y}px`, left: `${tools.x}px` }}>
                        <div className={'tools-btn'} onClick={makeBold}>
                            <b className={'tools-btn-text'}>B</b>
                        </div>
                        <div className={'tools-btn'} >
                            <i className={'tools-btn-text'} style={{ fontStyle: "italic" }}>I</i>
                        </div>
                        <div className={'tools-btn'} >
                            <u className={'tools-btn-text'} style={{ textDecoration: "underlined" }}>U</u>
                        </div>
                        <div className={'tools-btn'} >
                            <CustomSvg name={'link'} color={'#ffffff'} width={'30'} height={'30'} />
                        </div>
                    </div> : null
                }
                {
                    !isDetails ?
                        <div className={'editables'} >
                            {
                                fields.map(item => (
                                    <div className={'editables-item'} onMouseEnter={() => crossIconPosition(item.id)}>

                                        {
                                            item.type == "text" ?
                                                <p id={`${item.id}`} contentEditable={'true'} className={'editables-item-text'} onMouseUp={() => textSelected(item.id)} onInput={() => textChanged(item.id)}>{item.text}</p>
                                                :
                                                <img src={item.imagePreviewUrl} className={'editables-item-pic'} />
                                        }
                                        {
                                            imagePicker && item.id == fields[fields.length - 1].id ?
                                                <div className={"editables-item-browse"}>
                                                    <input type="file" accept="image/*" name="file" className={'editables-item-browse-input'} onChange={(e) => onChangeImageHandler(e)} />
                                                    <button onClick={() => setImagePicker(false)} title="cancel">cancel file upload</button>
                                                </div>
                                                : null
                                        }
                                        {
                                            cross == item.id ? <div className={'editables-item-cross'} onClick={() => setDeleteTarget(item.id)}>
                                        ...
                                        }
                                        {
                                            deleteTarget == item.id ?
                                                <div className={'delete'}>
                                                    <p className={'delete-title'}>Silmək istədiyinizdən əminsiniz?</p>
                                                    <div className={'delete-actions'}>
                                                        <button className={'delete-actions-btn'} onClick={() => deleteField(item.id)}>Hə</button>
                                                        <button className={'delete-actions-btn'} onClick={() => setDeleteTarget(null)}>Yox</button>
                                                    </div>
                                                </div>
                                                : null
                                        }
                                        {
                                            !imagePicker && item.id == fields[fields.length - 1].id ?
                                                <div className={'editables-item-add'} onClick={() => setModalFields(!modalFields)}>
                                                ...
                                                <div className={'modal'}>
                                                    <div className={'modal-btn'}>
                                                        <div className={'modal-btn-field'} onClick={addTextField}>
                                                            <CustomSvg name={'text'} color={'#ffffff'} width={'30'} height={'30'} />
                                                        </div>
                                                        <div className={'modal-btn-image'} onClick={addImageField}>
                                                            <CustomSvg name={'camera'} color={'#ffffff'} width={'30'} height={'30'} />
                                                        </div>
                                                    </div>
                                                </div> : null
                                        }
